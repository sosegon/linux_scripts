...
def convert_videos(root_folder, folder_name, videos_processed=[], overwrite=False):
    for elem_name in listdir(folder_name):
        elem_full_name = join(folder_name, elem_name)
        if isfile(elem_full_name):
            new_elem_full_name =  os.path.join(root_folder + '_converted', os.path.relpath(elem_full_name, root_folder)) if not overwrite else elem_full_name
            # Change extension to mp4
            new_elem_full_name = os.path.splitext(new_elem_full_name)[0] + '.mp4'
            ...
            try:
                ...
                if typefile is not None and typefile.find('video') != -1:
                    ...
                    # Get metadata to insert into converted video
                ...
                print('Error converting', elem_name, e)
                r = {
                    'status': 'ERROR',
                    'original_file': elem_full_name,
                    'original_size': file_size,
            ...
            convert_videos(root_folder, elem_full_name, videos_processed, overwrite)
...
def main():
    ...
    parser.add_argument('--log_file', '-l', type=str, default='./to_mp4_videos.csv', help='Log file')
    ...
    results = []
    convert_videos(folder_name, folder_name, results, overwrite)
    write_logs(log_file, results)
    print_summary(summarize_logs(results, ['CONVERTED', 'ERROR']))


if __name__ == '__main__':
    print('converting videos started')
    clock = Clock()
    clock.show()
    main()
    clock.hide()
    clock.print_time()
    print('converting videos completed')